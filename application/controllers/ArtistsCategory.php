...
class ArtistsCategory extends MY_Controller {
	...
	public function index()
	{
		...
		$data['header_title'] = 'Artists Category';
		$data['action'] = 'view';
		$data['filters'] = $filter;
		$this->load->template('ArtistsCategory',$data);
	}
	public function add(){
		$data['header_icon'] = 'fa fa-building text-purple';
		$data['header_title'] = 'Artists Category';
	...
	public function save(){
		...
		if($this->form_validation->run() == false){
			...
			$_POST['where'] = $slug;
			$_POST['slug'] = $this->slugify($this->input->post('name'),ARTISTS_CATEGORY," AND `slug`!='".$this->input->post('slug')."'");
			$data = $this->input->post();
			foreach($data as $key=>$val){
				$data[$key] = $this->db_input($val);
			}
			$response = json_decode($this->ArtistsCategoryModel->save($data),true);
		...
		if($slug==''){
			...
			redirect(base_url().'ArtistsCategory/edit/'.$this->input->post('slug'));
		}
	}
	public function DeleteImage($slug,$image){
		$response = json_decode($this->ArtistsCategoryModel->DeleteImage($slug,$image),true);
		if($response['status']==1){
			$this->session->set_flashdata('success',$response['message']);
		}
		else{
	...
	public function status(){
        ...
        $response = $this->ArtistsCategoryModel->status($slug,$status);
        echo $response;exit;
	}
	public function delete(){
		$slug = $this->input->post('slug');
        $response = $this->ArtistsCategoryModel->delete($slug);
        echo $response;exit;
	}
	public function edit($slug){
		$data = json_decode($this->ArtistsCategoryModel->edit($slug),true);
		$data['header_icon'] = 'fa fa-building text-purple';
		$data['header_title'] = 'Artists Category';
		$data['action'] = 'edit';
		$data['controller'] = $this;
	...
	public function select(){
		...
		$total_records = $this->ArtistsCategoryModel->count($status);
		$records = json_decode($this->ArtistsCategoryModel->select($status,$where,$orderby,$start,$length));
		$filtered_records = $this->ArtistsCategoryModel->count($status,$where);
		$record = array();
		foreach($records->records as $key=>$val){
			...
			$array[] = '<a href="'.base_url().'ArtistsCategory/edit/'.$this->db_output($val->slug).'" id="'.$this->db_output($val->slug).'" class="btn btn-xs btn-default text-blue"><i class="fa fa-edit"></i> Edit</a>
