...
class Partners extends MY_Controller {
	...
	public function index()
	{
		...
		$data['action'] = 'view';
		$data['filters'] = $filter;
		$this->load->template('Partners',$data);
	}
	public function add(){
		$data['header_icon'] = 'fa fa-building text-purple';
		$data['header_title'] = 'Partners';
	...
	public function save(){
		...
		if($this->form_validation->run() == false){
			...
			if($_FILES['image']['name']!=''){
				...
				$config['upload_path']   = $this->config->item('partners_dir');
				...
				if(!$this->upload->do_upload('image', false)){
					...
					if($slug==''){
						...
						redirect(base_url().'Partners/edit/'.$this->input->post('slug'));
					}
				}
				else{
					...
					$resizeObj = new ResizeImage($this->config->item('partners_dir').$image_name);
					$resizeObj -> resizeImage(135, 100, 'crop');
					$resizeObj -> saveImage($this->config->item('partners_dir').get_filename($image_name,false).'-135x100.'.get_extension($image_name),$this->config->item('partners_dir').$image_name, 100);
					$resizeObj -> resizeImage(360, 258, 'crop');
					$resizeObj -> saveImage($this->config->item('partners_dir').get_filename($image_name,false).'-360x258.'.get_extension($image_name),$this->config->item('partners_dir').$image_name, 100);
				}
			}

			$_POST['where'] = $slug;
			$_POST['slug'] = $this->slugify($this->input->post('name'),PARTNERS," AND `slug`!='".$this->input->post('slug')."'");
			$data = $this->input->post();
			foreach($data as $key=>$val){
				$data[$key] = $this->db_input($val);
			}
			$response = json_decode($this->PartnersModel->save($data),true);
	...
	public function DeleteImage($slug,$image){
		$response = json_decode($this->PartnersModel->DeleteImage($slug,$image),true);
		if($response['status']==1){
			$this->session->set_flashdata('success',$response['message']);
		}
		else{
	...
	public function status(){
        ...
        $response = $this->PartnersModel->status($slug,$status);
        echo $response;exit;
	}
	public function delete(){
		$slug = $this->input->post('slug');
        $response = $this->PartnersModel->delete($slug);
        echo $response;exit;
	}
	public function edit($slug){
		$data = json_decode($this->PartnersModel->edit($slug),true);
		$data['header_icon'] = 'fa fa-building text-purple';
		$data['header_title'] = 'Partners';
		$data['action'] = 'edit';
		$data['controller'] = $this;
	...
	public function select(){
		...
		$total_records = $this->PartnersModel->count($status);
		$records = json_decode($this->PartnersModel->select($status,$where,$orderby,$start,$length));
		$filtered_records = $this->PartnersModel->count($status,$where);
		$record = array();
		foreach($records->records as $key=>$val){
			...
			$array[] = '<a href="'.base_url().'Partners/edit/'.$this->db_output($val->slug).'" id="'.$this->db_output($val->slug).'" class="btn btn-xs btn-default text-blue"><i class="fa fa-edit"></i> Edit</a>
