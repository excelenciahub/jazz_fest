...
class Festival extends MY_Controller {
	...
	public function index()
	{
		...
		$data['header_title'] = 'Festival';
		$data['action'] = 'view';
		$data['filters'] = $filter;
		$this->load->template('Festival',$data);
	}
	public function add(){
		$data['header_icon'] = 'fa fa-building text-purple';
		$data['header_title'] = 'Festival';
	...
	public function save(){
		...
		if($this->form_validation->run() == false){
			...
			if($_FILES['image']['name']!=''){
				...
				$config['upload_path']   = $this->config->item('festivals_dir');
				...
				if(!$this->upload->do_upload('image', false)){
					...
					if($slug==''){
						...
						redirect(base_url().'Festival/edit/'.$this->input->post('slug'));
					}
				}
				else{
					...
					$resizeObj = new ResizeImage($this->config->item('festivals_dir').$image_name);
					$resizeObj -> resizeImage(135, 100, 'crop');
					$resizeObj -> saveImage($this->config->item('festivals_dir').get_filename($image_name,false).'-135x100.'.get_extension($image_name),$this->config->item('festivals_dir').$image_name, 100);
					$resizeObj -> resizeImage(360, 258, 'crop');
					$resizeObj -> saveImage($this->config->item('festivals_dir').get_filename($image_name,false).'-360x258.'.get_extension($image_name),$this->config->item('festivals_dir').$image_name, 100);
					$resizeObj -> resizeImage(800, 500, 'exact');
					$resizeObj -> saveImage($this->config->item('festivals_dir').get_filename($image_name,false).'-800x500.'.get_extension($image_name),$this->config->item('festivals_dir').$image_name, 100);
				}
			}
			$_POST['where'] = $slug;
			$_POST['slug'] = $this->slugify($this->input->post('name'),FESTIVAL_MASTER," AND `slug`!='".$this->input->post('slug')."'");
			$data = $this->input->post();
			foreach($data as $key=>$val){
				$data[$key] = $this->db_input($val);
			}
			$response = json_decode($this->FestivalModel->save($data),true);
	...
	public function DeleteImage($slug,$image){
		$response = json_decode($this->FestivalModel->DeleteImage($slug,$image),true);
		if($response['status']==1){
			$this->session->set_flashdata('success',$response['message']);
		}
		else{
	...
	public function status(){
        ...
        $response = $this->FestivalModel->status($slug,$status);
        echo $response;exit;
	}
	public function delete(){
		$slug = $this->input->post('slug');
        $response = $this->FestivalModel->delete($slug);
        echo $response;exit;
	}
	public function edit($slug){
		$data = json_decode($this->FestivalModel->edit($slug),true);
		$data['header_icon'] = 'fa fa-building text-purple';
		$data['header_title'] = 'Festival';
		$data['action'] = 'edit';
		$data['controller'] = $this;
	...
	public function select(){
		...
		$total_records = $this->FestivalModel->count($status);
		$records = json_decode($this->FestivalModel->select($status,$where,$orderby,$start,$length));
		$filtered_records = $this->FestivalModel->count($status,$where);
		$record = array();
		foreach($records->records as $key=>$val){
			...
			*/
			$array[] = '<a href="'.base_url().'Festival/edit/'.$this->db_output($val->slug).'" id="'.$this->db_output($val->slug).'" class="btn btn-xs btn-default text-blue"><i class="fa fa-edit"></i> Edit</a>';
