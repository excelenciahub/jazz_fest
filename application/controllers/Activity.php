...
class Activity extends MY_Controller {
	...
	public function index()
	{
		...
		$data['header_title'] = 'Activity';
		$data['action'] = 'view';
		$data['filters'] = $filter;
		$this->load->template('Activity',$data);
	}
	public function add(){
		$data['header_icon'] = 'fa fa-building text-purple';
		$data['header_title'] = 'Activity';
	...
	public function save(){
		...
		$this->form_validation->set_rules('event_date', 'Activity Date', 'required');
		...
		if($this->form_validation->run() == false){
			...
			if($_FILES['image']['name']!=''){
				...
				$config['upload_path']   = $this->config->item('activity_dir');
				...
				if(!$this->upload->do_upload('image', false)){
					...
					if($slug==''){
						...
						redirect(base_url().'Activity/edit/'.$this->input->post('slug'));
					}
				}
				else{
					...
					$resizeObj = new ResizeImage($this->config->item('activity_dir').$image_name);
					$resizeObj -> resizeImage(135, 100, 'crop');
					$resizeObj -> saveImage($this->config->item('activity_dir').get_filename($image_name,false).'-135x100.'.get_extension($image_name),$this->config->item('activity_dir').$image_name, 100);
					$resizeObj -> resizeImage(360, 258, 'crop');
					$resizeObj -> saveImage($this->config->item('activity_dir').get_filename($image_name,false).'-360x258.'.get_extension($image_name),$this->config->item('activity_dir').$image_name, 100);
					$resizeObj -> resizeImage(800, 500, 'exact');
					$resizeObj -> saveImage($this->config->item('activity_dir').get_filename($image_name,false).'-800x500.'.get_extension($image_name),$this->config->item('activity_dir').$image_name, 100);
				}
			}
			$_POST['where'] = $slug;
			...
			$response = json_decode($this->ActivityModel->save($data),true);
	...
	public function DeleteImage($slug,$image){
		$response = json_decode($this->ActivityModel->DeleteImage($slug,$image),true);
		if($response['status']==1){
			$this->session->set_flashdata('success',$response['message']);
		}
		else{
	...
	public function status(){
        ...
        $response = $this->ActivityModel->status($slug,$status);
        echo $response;exit;
	}
	public function delete(){
		$slug = $this->input->post('slug');
        $response = $this->ActivityModel->delete($slug);
        echo $response;exit;
	}
	public function edit($slug){
		$data = json_decode($this->ActivityModel->edit($slug),true);
		$data['header_icon'] = 'fa fa-building text-purple';
	...
	public function select(){
		...
		$total_records = $this->ActivityModel->count($status);
		$records = json_decode($this->ActivityModel->select($status,$where,$orderby,$start,$length));
		$filtered_records = $this->ActivityModel->count($status,$where);
		$record = array();
		foreach($records->records as $key=>$val){
			...
			$array[] = '<a href="'.base_url().'Activity/edit/'.$this->db_output($val->slug).'" id="'.$this->db_output($val->slug).'" class="btn btn-xs btn-default text-blue"><i class="fa fa-edit"></i> Edit</a>
