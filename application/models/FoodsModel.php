<?php if(!defined('BASEPATH')){ require_once("index.html");exit; }
    class FoodsModel extends CI_Model {
        ...
        public function DeleteImage($slug,$image){
            ...
            if($this->db->affected_rows()>0){
                $path1 = $this->config->item('foods_dir').$image;
                $path2 = $this->config->item('foods_dir').get_thumb($image,'135x100');
                $path3 = $this->config->item('foods_dir').get_thumb($image,'360x258');
                $path4 = $this->config->item('foods_dir').get_thumb($image,'800x800');
                unlink($path1);
                unlink($path2);
                unlink($path3);
                unlink($path4);
